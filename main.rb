module Mastermind
    ...
    class Game
        ...
        def print_board
            input_row_positions, result_row_positions = [[0, 1, 2, 3], [[0, 1], [2, 3]]]
            input_label_for_position = lambda{|position| @board[0][position] ? @board[0][position] : position}
            input_for_display = input_row_positions.map(&input_label_for_position).join(" ")
            result_label_for_position = lambda{|position| @board[1][position] ? @board[1][position] : position}
            result_row_for_display = lambda{|row| row.map(&result_label_for_position).join}
    ...
    class Computer < Player
        def create_secret_code!
            ...
            @game.secret_code = [options.shuffle[0], options.shuffle[0], options.shuffle[0], options.shuffle[0]]
